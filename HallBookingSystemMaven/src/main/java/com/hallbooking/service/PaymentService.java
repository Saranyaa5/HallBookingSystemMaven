...
public class PaymentService {
	 ...
	 private static String getValidCardNumber(Scanner scanner) {
        ...
        if (paymentMode.equals("UPI")) {
            success = paymentDAO.processPayment(bookingId, userId,paymentMode, paymentDetails);
        } else {
           success = paymentDAO.processPayment(bookingId, userId, paymentDetails, expiryDate, cvv, paymentMode);
        }


        
        if (success) {
            System.out.println("\n"+ConsoleColors.GREEN+"Payment Successful! Your hall booking is confirmed."+ConsoleColors.RESET);
        } else {
